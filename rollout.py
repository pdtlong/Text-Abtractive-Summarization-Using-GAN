...
#Đây là file dùng để để định nghĩa the decoder
import logging, os 
...
# Đây là hàm được xây dựng dựa trên  tf.contrib.legacy_seq2seq_attention_decoder
def roll_attention_decoder(decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell,
                      initial_state_attention=False, pointer_gen=True):
    """
    Các biến đầu vào:
        -decoder_inputs: Một list Tensors [batch_size x input_size].
        -initial_state: 2D Tensor [batch_size x cell.state_size].
        -encoder_states: 3D Tensor [batch_size x attn_length x attn_size].
        -enc_padding_mask: 2D Tensor [batch_size x attn_length] chứa 1s và 0s; cho biết vị trí encoder nào là padding (0) or a real token (1).
        -cell: rnn_cell.RNNCell Định nghĩa cell function and size.

        -initial_state_attention:
            + Nếu initial_state_attention = False (mặc định), Tại bước đầu tiên của decoder "Vector ngữ cảnh trước đó" là vector zero
            + Nếu initial_state_attention = True,  dùng initial_state để tính toán vectơ ngữ cảnh của bước trước đó. 
        -pointer_gen: boolean. If True, calculate the generation probability p_gen for each decoder step.

    Các biến trả về:
        -outputs: Một list cùng độ dài như decoder_inputs của 2D Tensors
        -shape [batch_size x cell.output_size]. Các vectơ đầu ra.
        -state: Trạng thái cuối cùng của decoder.Một tensor shape [batch_size x cell.state_size].
        -attn_dists: Một list chứa tensors với shape (batch_size,attn_length).Sự phân bố attention cho mỗi bước giải mã.
    ...
    with variable_scope.variable_scope("attention_decoder") as scope:
        ...
        states, attn_dists, p_gens, outputs = tf.map_fn(run,
        ...
        return tf.unstack(outputs), [tf.contrib.rnn.LSTMStateTuple(elem[0], elem[1]) for elem in tf.unstack(states)], tf.unstack(attn_dists), tf.unstack(p_gens)
