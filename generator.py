...
class Generator(object):
    ...
    def _add_placeholders(self):
        ...
        if FLAGS.pointer_gen:
            self._enc_batch_extend_vocab = tf.compat.v1.placeholder(tf.int32, [hps.batch_size, None],
                                                          name='enc_batch_extend_vocab')
            self._max_art_oovs = tf.compat.v1.placeholder(tf.int32, [], name='max_art_oovs')

        if FLAGS.seqgan:
            self.D_reward = tf.compat.v1.placeholder(shape=[hps.batch_size, FLAGS.max_dec_steps], dtype=tf.float32)
            self.rouge_reward = tf.compat.v1.placeholder(shape=[hps.batch_size, 1], dtype=tf.float32)
        ...
        self._dec_padding_mask = tf.compat.v1.placeholder(tf.float32, [hps.batch_size, hps.max_dec_steps],
    ...
    def _reduce_states(self, fw_st, bw_st):
        ...
        with tf.compat.v1.variable_scope('reduce_final_st'):
            ...
            w_reduce_c = tf.compat.v1.get_variable('w_reduce_c', [hidden_dim * 2, hidden_dim], dtype=tf.float32,
                                         initializer=self.trunc_norm_init)
            w_reduce_h = tf.compat.v1.get_variable('w_reduce_h', [hidden_dim * 2, hidden_dim], dtype=tf.float32,
                                         initializer=self.trunc_norm_init)
            bias_reduce_c = tf.compat.v1.get_variable('bias_reduce_c', [hidden_dim], dtype=tf.float32,
                                            initializer=self.trunc_norm_init)
            bias_reduce_h = tf.compat.v1.get_variable('bias_reduce_h', [hidden_dim], dtype=tf.float32,
                                            initializer=self.trunc_norm_init)
            # Apply linear layer
    ...
    def _add_seq2seq(self):
        ...
        with tf.compat.v1.variable_scope('seq2seq'):
            ...
            with tf.compat.v1.variable_scope('embedding'):
                self.embedding = tf.compat.v1.get_variable('embedding', [vsize, hps.emb_dim],
            ...
            with tf.compat.v1.variable_scope('output_projection'):
                self.w = tf.compat.v1.get_variable('w', [hps.hidden_dim, vsize], dtype=tf.float32,
                                         initializer=self.trunc_norm_init)
                self.v = tf.compat.v1.get_variable('v', [vsize], dtype=tf.float32, initializer=self.trunc_norm_init)
            ...
            if hps.mode in ['pretrain', 'train', 'eval']:
                ...
                with tf.compat.v1.variable_scope('loss'):
                    ...
                    tf.compat.v1.summary.scalar('loss', self._ML_loss)
                    tf.compat.v1.summary.scalar('sample_loss', self._RL_loss)
                    tf.compat.v1.summary.scalar('rouge_reward', tf.reduce_mean(self.rouge_reward))
                    if hps.seqgan:
                        tf.compat.v1.summary.scalar('D_reward_loss', self._GAN_loss)
                        tf.compat.v1.summary.scalar('D_reward', tf.reduce_mean(self.D_reward))
    ...
    def _get_token_prob(self, dist, num_samples=1, mode='argmax'):
        if mode == 'argmax':
            ...
            targets = tf.cast(tf.multinomial(tf.math.log(dist), num_samples=num_samples), tf.int32)
    ...
    def _rollout(self):
        hps = self._hps
        with tf.compat.v1.variable_scope("seq2seq/decoder", reuse=True) as scope:
    ...
    def build_graph(self):
        ...
        print('Đang xây dựng module graph...')
        t0 = time.time()
        ...
        print("Thời gian dùng để xây dựng module graph:",time.time() - t0," seconds", )
