...
def pretrain_generator(generator, generator_batcher, summary_writer, session):
    #Pretrain generator trước khi huấn luyện adversarial 
...
def pretrain_discriminator(discriminator, sess):
    ...
    for epoch in tqdm(range(train_max_epoch)):
        ...
        for it in range(dis_val_loader.num_batch):
            ...
            target = np.where(np.array(y_batch) == 1)[-1]
...
def adversarial_train(generator, discriminator, generator_batcher, discriminator_batcher,
    ...
    with sess_context_manager as sess:
        ...
        while True:
            ...
            print("RL reward cho rouge-L: %.3f" % np.mean(rouge_rewards))

            print("Tiến hành bước rollout ...")
            t0 = time.time()
            ...
            print("rollout bước: %.3fs" % (time.time() - t0))

            print("Bắt đầu tính D_rewards")
            feed_output_token = []
            ...
            for token in np.split(feed_output_token, FLAGS.rollout):
                ...
                # shape: [rollout_num * seqlen(số roll) * batch_size, 2]
            ...
            rewards = np.transpose(np.sum(ypred, 0)) / (1.0 * FLAGS.rollout)  
            ...
            # Train discriminator
            print("Bắt đầu huấn luyện discriminator...")
            for _ in tqdm(range(5)):
                ...
                # [batch_size, max_dec_steps]
                output_summary_token = np.transpose(np.squeeze(output_summary_token))

                # [batch_size, max_dec_steps]
                ground_truth = batch.target_batch  
                output_summary = []
