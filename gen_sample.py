...
def gen_dis_sample(train_pos_path, train_neg_path, vocab_path, n_dim=100, res_file='discriminator_train_data.npz'):
    ...
    print(" Độ dài bộ từ điển:", len(vocab_map))

    # Tải các positive samples
    train_pos = []
    ...
    print("\n các Positive samples:", len(train_pos))

    # Tải negative samples
    train_neg = []
    ...
    print("\n Các Negative samples: ", len(train_neg))
    np.savez(res_file, pos_summary_idx=np.array(train_pos), neg_summary_idx=np.array(train_neg))
    print("File đã được lưu vào: ", res_file)


def main(args):
    if not os.path.exists(args.data_dir):
        os.mkdir(args.data_dir)
    train_pos_path = args.decode_dir + "/References"
    train_neg_path = args.decode_dir + "/GAN_Generated"
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Chương trình sẽ tạo cả positive và negative samples")
